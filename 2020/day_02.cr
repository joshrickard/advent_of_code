...
alias PasswordPolicy = NamedTuple(num1: Int32, num2: Int32, letter: Char)

class OldJobPassword
  def self.valid?(password : String, policy : PasswordPolicy) : Bool
    occurences = password.count { |letter| letter == policy[:letter] }
    occurences >= policy[:num1] && occurences <= policy[:num2]
  end
end

class TobogganPassword
  def self.valid?(password : String, policy : PasswordPolicy) : Bool
    index1 = policy[:num1] - 1
    index2 = policy[:num2] - 1
    contains_first_letter = password.size > index1 && password[index1] == policy[:letter]
    contains_second_letter = password.size > index2 && password[index2] == policy[:letter]
    contains_first_letter ^ contains_second_letter
  end
end

class PasswordList
  REGEX = /(?<num1>\d+)-(?<num2>\d+) (?<letter>[a-z]): (?<password>.+)/

  def self.read(input : String, &) : Nil
    lines = input.split("\n").map { |l| l.strip }

    lines.each do |line|
      matches = REGEX.match(line)

      raise "couldn't match line #{line}" if matches.nil?
...
problem1_password_count = 0
problem2_password_count = 0
PasswordList.read(input) do |password, rules|
  problem1_password_count += 1 if OldJobPassword.valid?(password, rules)
  problem2_password_count += 1 if TobogganPassword.valid?(password, rules)
end

puts "Problem 1 valid passwords: #{problem1_password_count}"
puts "Problem 2 valid passwords: #{problem2_password_count}"
